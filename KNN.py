...
n_neighbors = range(11, 13, 1)
p = [1, 2]
gridparamKnn = [{'n_neighbors': n_neighbors, 'p': p}]

clf = GridSearchCV(estimator=KNeighborsClassifier(), param_grid=gridparamKnn, cv=5,
...
with open('bestKNN.txt', 'w') as file:
...
plt.xlabel('Minkowski power')
plt.ylabel('n_neighbors')
plt.colorbar()
plt.xticks(np.arange(len(p)), p, rotation=45)
...
#Export the model
model_filename = 'KNNmodel.joblib'
