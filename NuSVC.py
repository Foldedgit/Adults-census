...
nu = np.arange(0.26, 0.30, 0.2)
gamma = ['scale']
print(gamma)
gridparamSVC = [{'nu': nu, 'gamma': gamma}]

clf = GridSearchCV(estimator=NuSVC(kernel='rbf'), param_grid=gridparamSVC, cv=5,
                   scoring='roc_auc', n_jobs=-1, error_score='raise')

clf.fit(X_train, Y_train)
print("The best parameters are %s with a score of %0.2f"
      % (clf.best_params_, clf.best_score_))
scores = clf.cv_results_['mean_test_score'].reshape(len(nu), len(gamma))

with open('besNusvc.txt', 'w') as file:
...
plt.yticks(np.arange(len(nu)), nu)
