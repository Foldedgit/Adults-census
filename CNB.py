...
alpha = np.arange(4, 6, 1)
norm = ['True', 'False']

gridparamCNB = [{'alpha': alpha, 'norm': norm}]
clf = GridSearchCV(estimator=ComplementNB(), param_grid=gridparamCNB, cv=5,
                   scoring='roc_auc', n_jobs=-1, error_score='raise')
clf.fit(X_train, Y_train)
print("The best parameters are %s with a score of %0.2f"
      % (clf.best_params_, clf.best_score_))
scores = clf.cv_results_['mean_test_score'].reshape(len(alpha), len(norm))
#
# with open('bestCNB.txt', 'w') as file:
...
plt.ylabel('alpha')
plt.colorbar()
plt.xticks(np.arange(len(norm)), norm, rotation=45)
...
model_filename = 'CNBmodel.joblib'
